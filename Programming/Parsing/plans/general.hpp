// Oliver Kullmann, 18.11.2006 (Swansea)
/* Copyright 2006 - 2007 Oliver Kullmann
This file is part of the OKlibrary. OKlibrary is free software; you can redistribute
it and/or modify it under the terms of the GNU General Public License as published by
the Free Software Foundation and included in this library; either version 3 of the
License, or any later version. */

/*!
  \file Parsing/plans/general.hpp
  \brief Plans for the parsing modules


  \todo Move this module to part Programming. DONE


  \todo Create a milestones file.


  \todo Update namespaces
  <ul>
   <li> Either change the name of the module to "Parser", or change the
   namespace to "Parsing". </li>
  </ul>


  \todo Update namespace-usage


  \todo


  \todo Update the doxygen-documentation for this module.


  \todo Decide what to do with this module, and with parsing in general:
  <ul>
   <li> Is Boost::Spirit to be used? </li>
   <li> What about regular expressions in the extended Standard library? </li>
   <li> Are there new facilities in Boost 1.34 ? </li>
  </ul>


  \todo Transfer the tests to the new system (where are the current test functions used?)

*/

/*!
  \namespace OKlib::Parser
  \brief Components to support parsing
*/

namespace OKlib {
  namespace Parser {
  }
}

