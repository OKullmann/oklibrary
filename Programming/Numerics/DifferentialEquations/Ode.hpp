...
namespace Ode {
  ...
  public :
    ...
    void steps(const float_t delta, const count_t N = default_N) noexcept {
      ...
      for (count_t i = 0; i < N;) {
        y0 = std::fma(small_d, fp(x0,y0), y0);
  ...
  public :
    ...
    RK41d(const float_t x0, const float_t y0, const F_t fp, const f_t sol = f_t()) noexcept :
    fp(fp), sol(sol), x0(x0), y0(y0) {}
    ...
    void step(const float_t delta) noexcept {
      const float_t k1 = fp(x0,y0);
      const float_t d2 = delta / 2;
      const float_t xm = x0 + d2;
      const float_t k2 = fp(xm, std::fma(d2, k1, y0));
      const float_t k3 = fp(xm, std::fma(d2, k2, y0));
      x0 += delta;
      const float_t k4 = fp(x0, std::fma(delta, k3, y0));
    ...
    void steps(const float_t delta, const count_t N = default_N) noexcept {
      ...
      for (count_t i = 0; i < N;) {
        ...
        const float_t k3 = fp(xm, std::fma(d2, k2, y0));
        x0 = std::fma(++i, small_d, old_x0);
        const float_t k4 = fp(x0, std::fma(small_d, k3, y0));
