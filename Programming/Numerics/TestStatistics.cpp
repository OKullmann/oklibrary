...
int main(const int argc, const char* const argv[]) {
  ...
  {typedef EvalPoints<FP::float80> EP;
   typedef EP::point_t point_t;
   typedef std::vector<point_t> v_t;
   EP s;
   s.transfer_sorted(v_t{{0,0}});
   assert(s.S.N == 1);
   assert((s.S.xmin == point_t{0,0}));
   assert((s.S.xmax == point_t{0,0}));
   assert((s.S.ymin == point_t{0,0}));
   assert((s.S.ymax == point_t{0,0}));
   assert(s.S.xmid == 0);
   assert(s.S.ymid == 0);
   assert(s.S.xspan == 0);
   assert(s.S.yspan == 0);
   assert(FP::isnan(s.S.spanq));
   assert(s.S.ymean == 0);
   assert(s.S.ysd == 0);
   assert(s.S.ymed == 0);
   s.transfer_sorted(v_t{{1,2},{2,4}});
   assert(s.S.N == 2);
   assert((s.S.xmin == point_t{1,2}));
   assert((s.S.xmax == point_t{2,4}));
   assert((s.S.ymin == point_t{1,2}));
   assert((s.S.ymax == point_t{2,4}));
   assert(s.S.xmid == 1.5);
   assert(s.S.ymid == 3);
   assert(s.S.xspan == 1);
   assert(s.S.yspan == 2);
   ...
   assert(s.S.ysd == 1);
   assert(s.S.ymed == 3);
   s.transfer_sorted(v_t{{-1,1},{1,3},{3,0}});
   assert(s.S.N == 3);
   assert((s.S.xmin == point_t{-1,1}));
   assert((s.S.xmax == point_t{3,0}));
   assert((s.S.ymin == point_t{3,0}));
   assert((s.S.ymax == point_t{1,3}));
   assert(s.S.xmid == 1);
   assert(s.S.ymid == 1.5);
   assert(s.S.xspan == 4);
   assert(s.S.yspan == 3);
   assert(s.S.spanq == 0.75);
   assert(s.S.ymean == 4.0L/3);
   assert(s.S.ysd == FP::sqrt(14) / 3);
   assert(s.S.ymed == 1);
  }
}
