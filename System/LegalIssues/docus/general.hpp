...
// Oliver Kullmann, 23.9.2007 (Swansea)
   ...
   when creating this file, and to what degree the creator was actually involved
   in creating the file-content doesn't matter). </li>
  ...
  <ul>
   ...
   <li> Yet we do not add creation-information and licence-statements
   to html-files. </li>
  </ul>
  ...
  <ul>
   ...
   after the initial line with the script-info, and the follwoing
   line with the creation info. </li>
  ...
  the licence text LegalIssues/Header.h resp. LegalIssues/Header.s is inserted).
  Usage:
  <ul>
...
OKlib> for F in $(find * -name "*.hpp" -or -name "*.cpp" -or -name "*.h" -or -name "*.c")
  do 
   ...
   \endverbatim
   where in file Exceptions one finds those files which do not match the regular
   expression for the creation-information-line. </li>
