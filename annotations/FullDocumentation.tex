...
\label{sec:genprinexp}

All the experiments must be fully reproducible. Thus \emph{only} tools as provided by the \OKlibrary{} are to be used, \emph{NO} private code or private schemes in any form. Besides reproducibility, the point is to strengthen the \OKlibrary, by continuous improvement, not to weaken it by hiding.
...
We add the suffix ``\_XYZ123'' to the normal guard-macros, where ``XYZ123'' is a random string produced by \texttt{Random 6}.
