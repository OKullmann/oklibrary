...
namespace {
  ...
  using namespace OKlib::Rijndael;
...
namespace OKlib {
  ...
  int Rijndael::init(Mode mode,Direction dir,const UInt8 * key,KeyLength keyLen,UInt8 * initVector)
  {
    ...
    UInt8 keyMatrix[max_key_columns][4];
  ...
  int Rijndael::padEncrypt(const UInt8 *input, int inputOctets, UInt8 *outBuffer)
  {
    ...
    switch(m_mode)
      ...
      case ECB: 
        ...
        std::memcpy(block, input, 16 - padLen);
        std::memset(block + 16 - padLen, padLen, padLen);
        encrypt(block,outBuffer);
        break;
      case CBC:
        iv = m_initVector;
...
#if STRICT_ALIGN
            std::memcpy(iv, input, 16);
            std::memcpy(outBuf, block, 16);
#else
  ...
  int Rijndael::padDecrypt(const UInt8 *input, int inputOctets, UInt8 *outBuffer)
  {
    ...
    case ECB:
      ...
      std::memcpy(outBuffer, block, 16 - padLen);
      break;	
    case CBC:
      std::memcpy(iv, m_initVector, 16);
      /* all blocks but last */
  ...
  void Rijndael::keySched(UInt8 key[max_key_columns][4])
  {
    int rconpointer = 0;
    ...
    UInt8 tempKey[max_key_columns][4];
    
    // Copy the input key to the temporary key matrix
    
    for(int j = 0; j < uKeyColumns; ++j)
      {
    ...
    for(int j = 0; (j < uKeyColumns) && (r <= m_uRounds); )
      {
    ...
    while(r <= m_uRounds)
      {
        ...
        if (uKeyColumns != 8)
          {
            for (int j = 1; j < uKeyColumns; ++j)
              {
                *((UInt32*)tempKey[j]) ^= *((UInt32*)tempKey[j-1]);
              }
          } else {
          for (int j = 1; j < uKeyColumns/2; ++j)
            {
              *((UInt32*)tempKey[j]) ^= *((UInt32*)tempKey[j-1]);
            }
          ...
          for(int j = uKeyColumns/2 + 1; j < uKeyColumns; ++j)
            {
              *((UInt32*)tempKey[j]) ^= *((UInt32*)tempKey[j-1]);
            }
        }
  ...
  void Rijndael::encrypt(const UInt8 a[16], UInt8 b[16])
  {
    ...
    *((UInt32*)(b +12)) = *((UInt32*)T1[temp[3][0]])
      ...
      ^ *((UInt32*)T3[temp[1][2]]) 
      ^ *((UInt32*)T4[temp[2][3]]);
    for(unsigned r = 1; r < m_uRounds-1; ++r)
      {
