  ...
  <ul>
   ...
   <li> Solving 20 random keys (avg number of conflicts/nodes):
    <ul>
     ...
     <li> canonical CNF (minisat-2.2.0: ??, OKsolver_2002: ??). </li>
    </ul>
   </li>
...
1 4 2624 23840 15.08476   1 1050555 1986753 2021.65 344645057 71.6 0.0105  0.01
        cfl r    k
1 147441100 4 10.5
   \endverbatim
...
1 4 2616 23968 69504 1.325   1 56743.6 128 6521081   0 0.05 26.2   NA 20928
...
shell> echo "n  c  l  t  sat  nds  r1  r2  pls  ats h file n2cr  dmcl dn  dc  dl snds qnds mnds  tel  oats  n2cs  m2cs r k" > oksolver_results;
for k in $(seq 1 20); do
    OKP=~/Work/OKlibrary/OKplatform/; cat oksolver_r${r}_k${k}.result | awk -f extract_bash_time_oksolver.awk | awk -f ${OKP}/OKsystem/OKlib/Experimentation/ExperimentSystem/SolverMonitoring/ExtractOKsolver.awk | awk " { print \$0 \"  $r  $k\" }";
done >> oksolver_results;
shell> oklib --R
E = read.table("oksolver_results", header=TRUE)
EM = aggregate(E, by=list(r=E$r), FUN=mean)
...
1    0 128 576 1920 0.25    0  0.3   0    0    0    0 4 10.5
   \endverbatim
   where "extract_bash_time_oksolver.awk" is:
   \verbatim
/^c running_time\(sec\)/ { }
...
EM
  r   n    c        t sat      cfs     dec    rts       r1 mem  ptime stime
1 4 576 5536 21.28601   1 982518.8 1116844 1927.3 71803310 8.8 0.0035  0.01
       cfl r    k
1 18830930 4 10.5
   \endverbatim
   and using "real" time from "time" command:
...
1 4 568 5664 27076 3.18   1 4134272 128 10648856   0 0.1 35.7   NA  448    0
   dn  dc   dl   snds qnds   mnds tel oats n2cs m2cs r    k
1 128 576 1920 1528.5    0 4259.2   0    0    0    0 4 10.5
   \endverbatim
   and using "real" time from "time" command:
   \verbatim
shell> echo "n  c  l  t  sat  nds  r1  r2  pls  ats h file n2cr  dmcl dn  dc  dl snds qnds mnds  tel  oats  n2cs  m2cs r k" > oksolver_results;
...
EM
  r   n    c        t sat     cfs     dec     rts       r1 mem ptime stime
1 4 576 3440 27.15832   1 1331979 1655787 2558.05 79038583 8.2     0 0.001
       cfl r    k
1 24357557 4 10.5
   \endverbatim
   and using "real" time from "time" command:
...
EM
  r   n    c     l    t sat     nds  r1       r2 pls ats     h file n2cr dmcl
1 4 568 3568 16048 5.27   1 6566953 128 17938032   0 0.1 38.35   NA  448    0
   dn  dc   dl    snds qnds    mnds tel oats n2cs m2cs r    k
1 128 576 1920 7552.55    0 13192.2   0  9.7    0    0 4 10.5
   \endverbatim
   and using "real" time from "time" command:
   \verbatim
shell> echo "n  c  l  t  sat  nds  r1  r2  pls  ats h file n2cr  dmcl dn  dc  dl snds qnds mnds  tel  oats  n2cs  m2cs r k" > oksolver_results;
...
[[10,960]]
  \endverbatim
   ...
   <li> This todo must be updated with non-randomised results! </li>
  </ul>
