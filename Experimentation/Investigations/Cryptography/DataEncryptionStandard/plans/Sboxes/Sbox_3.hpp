...
/*!
  \file Investigations/Cryptography/DataEncryptionStandard/plans/Sboxes/Sbox_3.hpp
  \brief On investigations into Sbox three of the Data Encryption Standard
  ...
  <ul>
   <li> Generating the full CNF representation:
    <ol>
     <li> The CNF-file "DES_Sbox_3_fullCNF.cnf" is created by the Maxima-function
     output_dessbox_fullcnf_stdname(3) in
...
10 960
     ...
     bf2relation_fullcnf_fcs(des_sbox_bf(3),6). </li>
    </ol>
   </li>
   <li> The minimum CNF representation has at most 68 clauses. See
   "Using weighted MaxSAT to compute small CNFs". </li>
  </ul>
  

  \todo Using weighted MaxSAT to compute small CNFs (mincl_rinf <= 68)
  <ul>
