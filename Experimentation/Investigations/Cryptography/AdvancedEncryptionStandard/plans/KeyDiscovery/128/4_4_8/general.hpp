...
/*!
  \file Investigations/Cryptography/AdvancedEncryptionStandard/plans/KeyDiscovery/128/4_4_8/general.hpp
  ...
  <li> We consider AES which has 4 rows, 4 columns, using the 8-bit
   ...
   aes(r,4,4,8) into SAT. </li>
   <li> aes(r,4,4,8) takes a 128-bit plaintext and 128-bit key and outputs a
   128-bit ciphertext. </li>
   <li> aes(r,4,4,8) applies the following operations:
    <ol>
     <li> Key schedule which takes the key and generates r+1 128-bit round
     keys. </li>
...
K_(i,4,k) := S-box(K_(i-1,1,2)) + sum(K_(i-1,4,l),l,1,4)
   ...
   where
    <ul>
     ...
     round-key considered as a 4x4 matrix. </li>
    </ul>
   </li>
...
MixColumns(I_8) := Mul03(I_5) + I_6 + I_7 + Mul02(I_8)
   ...
   <li> The inverse MixColumns operation is a permutation from ({0,1}^8)^2 to
   ({0,1}^8)^2, which we consider to be defined as:
   \verbatim
...
InvMixColumns(I_8) := Mul11(I_5) + Mul13(I_6) + Mul9(I_7) + Mul14(I_8)
   ...
   where
    <ul>
     <li> I_i is the i-th 8-bit word in the input; </li>
     <li> MulX is a permutation over {0,1}^8 representing multiplication
