...
[1] 1.468404
       ...
       <li> Fitting the data:
        <ul>
         <li> The following models seem to fit the data well
         (averaging values for each round over 20 random keys):
          <ul>
           <li> Time taken vs rounds: linear model for all translations.
           <li> Conflicts vs rounds:
            <ul>
             <li> linear model for 1-soft translations; </li>
             <li> logarithmic model for minimum translation. </li>
            </ul>
           </li>
          </ul>
         </li>
         <li> For each round, the times and conflicts vary greatly across keys;
         the standard deviation for time and conflicts is roughly the average
         in both cases. </li>
...
Lt_canon = lm(EA_canon$t ~ EA_canon$r)
plot(EA_canon$r,EA_canon$t)
lines(EA_canon$r, predict(Lt_canon))
########## Time taken vs rounds (1-base) ###########
Lt_1base = lm(EA_1base$t ~ EA_1base$r)
plot(EA_1base$r,EA_1base$t)
lines(EA_1base$r, predict(Lt_1base))
########## Time taken vs rounds (min) ###########
Lt_min = lm(EA_min$t ~ EA_min$r)
plot(EA_min$r,EA_min$t)
lines(EA_min$r, predict(Lt_min))

########## Conflicts vs rounds (canonical) ###########
Lcfs_canon = lm(EA_canon$cfs ~ log(EA_canon$r))
plot(EA_canon$r, EA_canon$cfs)
lines(EA_canon$r,predict(Lcfs_canon))
########## Conflicts vs rounds (1-base) ###########
Lcfs_1base = lm(EA_1base$cfs ~ log(EA_1base$r))
plot(EA_1base$r, EA_1base$cfs)
lines(EA_1base$r,predict(Lcfs_1base))
########## Conflicts vs rounds (min) ###########
Lcfs_min = lm(EA_min$cfs ~ EA_min$r)
plot(EA_min$r, EA_min$cfs)
lines(EA_min$r,predict(Lcfs_min))
         \endverbatim
         </li>
        </ul>
       </li>
       <li> All experiments take less than 15s, so we may be encountering
       small scale effects. </li>
      </ul>
     </li>
    </ul>
   </li>
