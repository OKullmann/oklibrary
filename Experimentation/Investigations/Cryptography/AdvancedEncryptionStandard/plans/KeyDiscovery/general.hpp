  ...
  <ul>
   ...
   <li> 4-bit (field size) instances:
    <ul>
     ...
     or quadratic; there is high variance) in r (the number of rounds). </li>
     <li> Further experiments should be done here to find a polynomial
     with good fit and then test these predictions against instances with
     larger numbers of rounds. </li>
     <li> Across all 4-bit instances so far, all translations perform
     comparably (there is no clear "better" translation).  In general
     the 1-base translation performs best, but only by a small margin. </li>
    </ul>
   </li>
   <li> 8-bit (field size) instances:
    <ul>
     <li> For instances of the form AES(r,1,1,8), AES(r,2,1,8), and
     AES(r,1,2,8), the 1-soft translations perform better by a considerable
     margin. </li>
