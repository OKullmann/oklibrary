...
/*!
  ...
  <ul>
   ...
   prohibitively large, such as is the case with the Sbox with > 100000 clauses.
   </li>
   ...
   <li> Given an integer representation, the full CNF representation of the 
   boolean function for multiplication by a constant factor n in Rijndael's
...
N=2;cat AES_byte_field_mul_full_${N}.pi | while read x; do CLAUSECOUNT=`echo $x | wc -w`; echo `expr $CLAUSECOUNT - 1`; done | sort > AES_byte_field_mul_full_${N}.stats; cat AES_byte_field_mul_full_${N}.stats | head -n 1; cat AES_byte_field_mul_full_${N}.stats | tail -n 1
   ...
   <li> It would be nice here to be able to read the clause-sets into the maxima
   system, where we have statistics_fcl etc. (see "Input and output" in 
