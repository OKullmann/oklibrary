...
/*!
  ...
  <ul>
   ...
   <li> The report http://arxiv.org/abs/1406.7398 "A framework for good SAT
   translations, with applications to CNF representations of XOR constraints"
   solves the mentioned complexity-problems below. </li>
  </ul>
...
output_gen_2xor_stdname(n);
   ...
   TODO: update according to arXiv report
   \verbatim
...
10 [{{}},[68,86,96,98,118,120,118,138,140,138,158,160,158,106,1]]
  ...
  <ul>
   <li> TODO: update according to arXiv report </li>
...
whardness_wpi_cs(F5,P5);
  ...
  \todo Generator parityphp_cs(p,q,s)
  <ul>
   ...
   <li> Now we assume that n= 2 q, and we use additional variable v_1,...,v_p
   for some p >= 1. </li>
   <li> Finally we add the parity constraints
    <ol>
     <li> v_1+...v_p + php(1,1)+...+php(1,q) = 0 </li>
     <li> v_1+...+v_{p-1} - v_p + php(1,q+1)+...+php(1,n) = 0 </li>
    </ol>
    which makes the whole system unsatisfiable.
   </li>
   <li> The problems are generated by output_parityphp_stdname(p,q,s),
   where s=0 means, that each of the two parity constraints is translated
   in the standard way on its own (X_1 in the report), while for s=1 the
   improved translation for the two constraints together (X_2 in the report)
   is used. </li>
  </ul>
