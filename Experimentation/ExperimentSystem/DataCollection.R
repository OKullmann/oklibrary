...
# Given a data.frame with columns column_keys and column_vals, and
# columns given in the list constant_columns, constructs a new data.frame
# with a column for every unique value in the column keys column, taking
# values from the column_values column as the rows for each value of
# column_keys.
#
# TODO: Improve explanation.
rows2columns_df = function(D, column_keys, column_vals, constant_columns) {
  df = do.call(cbind,
    append(
      Map(
        function(C) D[D[column_keys] == C,][column_vals], 
        as.list(t(unique(D[column_keys])))),
      Map(function(C) unique(D[C]), constant_columns))
  )
  colnames(df) = append(as.list(t(unique(D[column_keys]))),constant_columns)
  df
}
# For example, if we have files generated by
#
# shell> ${OKlib}/Experimentation/Investigations/BooleanFunctions/analyse_random_permutations 2 1 3
#
# then run:
#
# R> E = read_experiment_dirs("random_perm", list("e","seed"), "Permutation_full.cnf_primes_stats", header=TRUE, skip=2)
...
# then we have:
#
# R> rows2columns_df(E, "length", "count", list("e","seed"))
#    0 1 2 3 4 e seed
# 1  0 0 2 8 0 2    1
# 6  0 0 2 8 0 2    2
# 11 0 0 2 8 0 2    3
#
# Note that the assumption here is that for each seed, there are the same
# values of length, which each have potentially different values for
# count.
#
