// Oliver Kullmann, 8.3.2006 (Swansea)

/*!
  \file Graphs/Matchings/plans/milestones.hpp

  \module_version Graphs/Matchings 0.0.2 (8.3.2006)

  \par Version 0.0.3

  \par
   In Graphs/Matchings/plans/general.hpp the following topics are handled:
    - Update namespaces
    - Correction : DONE

  Refine and update the following milestones.


  \par Version 0.1

  \par
   In Graphs/Matchings/plans/general.hpp the following topics are handled:
    - Literature and software review
    - Concepts for bipartite graphs


  \par Version 0.2

  \par
   In Graphs/Matchings/plans/general.hpp the following topics are handled:
    - First implementations


  \par Version 0.3

  \par
   In Graphs/Matchings/plans/general.hpp the following topics are handled:
    - Injectivity constraints


  \par Version 0.4

  \par
   In Graphs/Matchings/plans/general.hpp the following topics are handled:
    - Online


  \par Version 0.5

  \par
   In Graphs/Matchings/plans/general.hpp the following topics are handled:
    - Counting


  \par Version 0.6

  \par
   In Graphs/Matchings/plans/general.hpp the following topics are handled:
    - Surplus


  \par Version 1.0
  A variety of efficient implementations are available, fulfilling the
  needs of the module InjectivityConstraints and MatchingAutarkies.



  -------------------------------------------------------------------------------------------------------------------------------------

  \par Version history

   - 0.0.2 : 8.3.2006; initial version number (only some plans).

*/

