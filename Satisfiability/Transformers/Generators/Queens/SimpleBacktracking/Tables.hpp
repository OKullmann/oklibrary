...
/* Lookup tables for the 2-sweep algorithm.
   ...
   The struct SignificantZeroIndices contains two const arrays. The first/second
   array stores indices of the most/least significant zero in a bit-representation of
   1<<bits_num unsigned values, where bits_num is the size of a board part. Two
   instances of the struct SignificantZeroIndices are made: lower_indicies stores
   indices for the lower part (bottom and lower), while upper_indicies reflects the
   upper part (upper, top). The function bottom takes the value of
   lower_indicies.first, etc. An unsigned long representation of the bitset b from
   DoubleSweep is divided into two parts each of which is used as an index for
   arrays lower_indicies/upper_indicies. For the upper part, shift_value is
   additionally used to take into account the lower part while assigning upper and top.
   For example, if N=17 and b is 01000111 111000100 (binary), then the unsigned long
...
namespace Tables {
  ...
  constexpr sizet lower_bits_num = Dimensions::N%2 == 1 ? Dimensions::N/2 + 1 : Dimensions::N/2;
  ...
  struct SignificantZeroIndices
  {
    ...
    constexpr SignificantZeroIndices() : first(), second() {
