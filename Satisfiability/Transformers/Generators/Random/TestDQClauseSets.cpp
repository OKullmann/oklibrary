...
int main(const int argc, const char* const argv[]) {
  ...
  {const auto [S,V] = create_dependencies(dep_edges{{1,0}}, block_v{{3,Q::both},{1,Q::fa},{{2,3},Q::ex}}, DepOp::from_scratch);
   assert(S.size() == 2);
   const auto f2 = S.find({1});
   assert(f2 != S.end());
   const auto f3 = S.find({});
   assert(f3 != S.end());
  ...
  {const auto [S,V] = create_dependencies(dep_edges{{1,1}}, block_v{{3,Q::both},{1,Q::fa},{{2,3},Q::ex}}, DepOp::from_scratch);
   assert(S.size() == 2);
   const auto f2 = S.find({});
   assert(f2 != S.end());
   const auto f3 = S.find({1});
   assert(f3 != S.end());
   assert((V == Dvector{nullptr,nullptr,&*f2,&*f3}));
  }
  ...
  {const auto [S,V] = create_dependencies(dep_edges{{2,0},{8,0},{10,5}}, block_v{{11,Q::both},{1,Q::ex},{{2,2},Q::fa},{{3,7},Q::ex},{{8,10},Q::fa},{{11,11},Q::ex}}, DepOp::from_scratch);
   assert(S.size() == 3);
   const auto f1 = S.find({2,8});
   assert(f1 != S.end());
   const auto f3 = S.find({});
   assert(f3 != S.end());
   const auto f7 = S.find({10});
   assert(f7 != S.end());
   assert((V == Dvector{nullptr,&*f1,nullptr,&*f3,&*f3,&*f3,&*f3,&*f7,nullptr,nullptr,nullptr,&*f3}));
  }

  {const auto [S,V] = create_dependencies(dep_edges{}, block_v{{1,Q::ex},{1,Q::ex}}, DepOp::subtract);
   assert(S.size() == 1);
   const auto f = S.find({});
   assert(f != S.end());
   assert((V == Dvector{nullptr, &*f}));
  }
  {const auto [S,V] = create_dependencies(dep_edges{}, block_v{{2,Q::both},{1,Q::fa},{{2,2},Q::ex}}, DepOp::subtract);
   assert(S.size() == 1);
   const auto f = S.find({1});
   assert(f != S.end());
   assert((V == Dvector{nullptr,nullptr,&*f}));
  }
  {const auto [S,V] = create_dependencies(dep_edges{{1,0}}, block_v{{2,Q::both},{1,Q::fa},{{2,2},Q::ex}}, DepOp::subtract);
   assert(S.size() == 1);
   const auto f = S.find({});
   assert(f != S.end());
   assert((V == Dvector{nullptr,nullptr,&*f}));
  ...
  {const auto [S,V] = create_dependencies(dep_edges{{1,1}}, block_v{{3,Q::both},{1,Q::fa},{{2,3},Q::ex}}, DepOp::subtract);
   assert(S.size() == 2);
   const auto f2 = S.find({1});
   assert(f2 != S.end());
   const auto f3 = S.find({});
  ...
  {const auto [S,V] = create_dependencies(dep_edges{{1,0},{1,1}}, block_v{{3,Q::both},{1,Q::fa},{{2,3},Q::ex}}, DepOp::subtract);
   assert(S.size() == 1);
   const auto f = S.find({});
   assert(f != S.end());
   assert((V == Dvector{nullptr,nullptr,&*f,&*f}));
  }
  {const auto [S,V] = create_dependencies(dep_edges{{2,2},{8,6},{10,6}}, block_v{{11,Q::both},{1,Q::ex},{{2,2},Q::fa},{{3,7},Q::ex},{{8,10},Q::fa},{{11,11},Q::ex}}, DepOp::subtract);
   assert(S.size() == 3);
   const auto f1 = S.find({});
   assert(f1 != S.end());
   const auto f4 = S.find({2});
   assert(f4 != S.end());
