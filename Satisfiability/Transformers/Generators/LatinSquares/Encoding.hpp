...
namespace Encoding {
  ...
  public :
    ...
    constexpr VarEncoding(const Statistics::Param ps, const Options::SymP s,
                          const Options::EAloP e, const Options::EulP eul,
                          const Options::PrimeP prim) noexcept :
    N(ps.N), k(ps.k), nvc(numvarscls(ps,s,e,eul,prim)), symopt(s), ealoopt(e),
    eulopt(eul), primopt(prim) {}
    ...
    constexpr Statistics::var_t operator()(const Statistics::dim_t i,
                                           const Statistics::dim_t j,
                                           const Statistics::dim_t eps,
                                           const Statistics::dim_t p)
      const noexcept {
      assert(i < N);
      assert(j < N);
      assert(eps < N);
      ...
      if (symopt == Options::SymP::full) {
        ...
        if (p == 0) {
          ...
          const Statistics::var_t n_prev_cells = i>=j ? (j-1)*(N-2) :
            (j-2)*(N-2) + (N-1);
          const Statistics::var_t v = 1 + n_prev_lines + n_prev_cells +
            eps_adj(i,j,eps);
          assert(v <= nvc.nls);
          return v;
        }
          ...
          const Statistics::var_t v = 1 + n_prev_ls + n_prev_lines +
            n_prev_cells + (j==0 ? eps_adj(i,j,eps) : eps_adj(j,eps));
          assert(v <= nvc.nls);
          return v;
        }
      }
    ...
    constexpr Statistics::var_t operator()(const Statistics::dim_t i,
      ...
      if (symopt == Options::SymP::full) {
        ...
        const Statistics::var_t v = 1+nvc.nls + n_prev_es + n_prev_lines +
          n_prev_cells + index(eps,N);
        ...
        if (p >= 1) {
          ...
          const Statistics::var_t n_prev_es = q * nvc.nbes1 +
            (index(pq)-q) * nvc.nbes2;
          const Statistics::var_t n_prev_lines = (i-1) * N * (N-1) * (N-2) -
            (i-1) * 2 * (N-2);
          const Statistics::var_t n_prev_cells = j==0 ? 0  :
            j * (N-1) * (N-2) - 2 * (N-2);
          const Statistics::var_t v = 1+nvc.nls + n_prev_es + n_prev_lines +
            n_prev_cells + (j==0 ? index_adj2(i,j,eps) : index_adj(j, eps));
          assert(nvc.nls < v and v <= nvc.n0);
          return v;
        }
        else {
          ...
          assert(eps.x != i);
          const Statistics::var_t n_prev_es = (q-1) * nvc.nbes1 +
            (index(pq)-(q-1)) * nvc.nbes2;
          const Statistics::var_t n_prev_lines = (i-1) * ((N-2)*(N-2)*(N-2) +
                                                          (N-1)*(N-2));
          const Statistics::var_t n_prev_cells = j<=i ? (j-1) * (N-2) * (N-2) :
            (N-1)*(N-2) + (j-2) * (N-2) * (N-2);
          const Statistics::var_t v = 1+nvc.nls + n_prev_es + n_prev_lines +
            n_prev_cells + index_adj(i, j, eps);
          assert(nvc.nls < v and v <= nvc.n0);
          return v;
        }
      }
    ...
    static constexpr Statistics::var_t eps_adj(Statistics::var_t i,
                                               Statistics::var_t j,
                                               const Statistics::var_t eps)
      noexcept {
      if (i == j) {
        if (eps < i) return eps;
        else return eps-1;
    ...
    static constexpr Statistics::var_t eps_adj(const Statistics::var_t j,
                                               const Statistics::var_t eps)
      noexcept {
    ...
    constexpr Statistics::var_t index_adj(const Statistics::var_t j,
                                          const ValPair eps) const noexcept {
    ...
    constexpr Statistics::var_t index_adj(const Statistics::var_t i,
                                          const Statistics::var_t j,
                                          const ValPair eps) const noexcept {
    ...
    constexpr Statistics::var_t index_adj2(const Statistics::var_t i,
                                           const Statistics::var_t j,
                                           const ValPair eps) const noexcept {
      assert(i != 0 and j == 0);
