// Oliver Kullmann, 29.1.2008 (Swansea)
/* Copyright 2008 Oliver Kullmann
This file is part of the OKlibrary. OKlibrary is free software; you can redistribute
it and/or modify it under the terms of the GNU General Public License as published by
the Free Software Foundation and included in this library; either version 3 of the
License, or any later version. */

/*!
  \file Satisfiability/Variations/plans/general.hpp
  \brief Plans for the development of components for variations on (generalised) SAT


  \todo Update namespaces.


  \todo Create module on NaeSat.


  \todo Create module on XSat.


  \todo Module on counting
  <ul>
   <li> Perhaps here we should have a sub-module Counting? </li>
   <li> There we should implement the algorithm for approximating
   the number of falsifying assignments by sampling. </li>
   <li> Of course, counting algorithms are also obtained by representing
   all solutions, which is under AllSolutions/plans/general.hpp ? </li>
   <li> We need to find a clear structure where to place what. </li>
   <li> See ComputerAlgebra/Satisfiability/Lisp/Counting/plans/general.hpp.
   </li>
  </ul>

*/

