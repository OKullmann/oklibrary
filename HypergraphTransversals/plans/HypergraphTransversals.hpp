...
/*!
  ...
  \todo What is known meanwhile about the existence of polynomial
  delay enumeration of Tr(G) for hypergraphs G? </li>

  \todo For example Wahlstroem has an algorithm for enumerating
  Tr(G) for 3-uniform G --- worth implementing? Likely one needs
  to extract a heuristical generalisation.

  \todo The dual problem to hypergraph transversal enumeration
  is enumeration of all edge covers of a hypergraph ---
  has this formulation advantages / disadvantages?

  \todo The complementary problem (by hyperedge-complementation)
