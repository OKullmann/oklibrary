...
public :
  ...
  virtual void compute_mean() {
    ...
    std::vector<double> medianassert2(*vecndsp);
    {
      ...
      assert(boost::test_toolbox::close_at_tolerance<double>(10 * std::numeric_limits<double>::epsilon())(nds, std::accumulate(vecndsp -> begin(), vecndsp -> end(), double(0)) / count));
      ...
      if (not boost::test_toolbox::close_at_tolerance<double>(5 * std::numeric_limits<double>::epsilon())(mednds, median2)) {
	...
	std::cerr << "\n";
	assert(boost::test_toolbox::close_at_tolerance<double>(5 * std::numeric_limits<double>::epsilon())(mednds, median2));
      }
    }
    delete vecndsp;
    vecndsp = 0;
