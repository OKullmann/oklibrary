    ...
    <ul>
     <li>Post-order traversal, recursively do the following steps from leaves.
      <ol>
       ...
       only one child (left child), the offset is the preliminary x coordinate 
       of current node minus 1 (1 is the result of the minimum distance 2 
       divided by 2). The offset of leaves are 0.</li>
   ...
   <li>An example:
    <ul>
     ...
     which has 12 nodes.
	  ...
	  of C is 0-1=-1.</li>
	  <li>F is the same as D, so px=0, offset=0, px is used to indicate the 
	  preliminary x coordinate, it will be used in all of the following 
	  example.</li>
	  <li>G is the right child of E, so px=0+2=2, offset=0.</li>
	  <li>E is the right child of C, and the parent of F and G, so px=0+2=2, 
	  offset=2-(0+2)/2=1.</li>
	  <li>Determine the seperation between the right most node of left 
	  subtree and the left most node of right subtree in same level, in the 
	  case,they are D and F .So use the method given above to calculate the 
	  distance between the two nodes, (0+1)-(0+(-1))=2, thus no further move 
	  needed.</li>
