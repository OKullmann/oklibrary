...
is(Csa);
  ...
  UCP
  <ul> 
   <li> Where to place such definitions? </li>
   <li> For now they have been added to the comments of 
   cardinality_totalizer_cl and cardinality_cl, however the comments for
   these functions are far too large, therefore these things
   should be moved to the docus? </li>
   <li> MG also has a proof that any satisfying assignment of
   the CNF F generated by cardinality_totalizer_cl (assuming S
   is without complimentary literals) must be total which should also be 
   placed in the docus once properly created. </li>
   <li> DONE The basic definitions and algorithms for CNF representations of
