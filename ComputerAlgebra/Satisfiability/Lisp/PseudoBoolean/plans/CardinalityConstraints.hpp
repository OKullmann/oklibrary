...
draw_lrt_dbl(unary_addition_tree_0_lrt(5),d:inf);
   ...
   intimately related, how to specify the auxiliary variables.
    <ul>
     ...
     variables representing the unary sum of the list E[a],...,E[b]. </li>
     <li> OK: what is "E" here? (*reflection* is needed! and this starting
     from the basics) And what is the meaning of "One method" ? </li>
     <li> Such a method allows for variables names which allow for 
     a simple recursion generation, but have names which can be referred to
     without reference to the underlying tree, which is useful when
     describing and understanding them. </li>
     <li> OK: which recursion? We need *discussion*. </li>
    </ul>
   </li>
   <li> To label the tree generated by "unary_addition_tree_0_lrt" with 
   variables, we have the following function, which takes a binary labelled 
   rooted tree, where 
OK: what is the "following function" ? which "tree" ?
    <ol> 
     <li> every label of the rooted tree is a list [n] where
OK: what is "[n]" ?
     n is a natural number, and </li>
     <li> for any two children with the same parent and labels [a] and [b], 
     we have that the parent has the label [a+b]. </li>
    </ol>
   \verbatim
unary_addition_lrt_p(T) := listp(T) and
  if length(T) = 1 then listp(T[1]) and integerp(T[1][1]) and is(T[1][1] >= 1)
  else if length(T) = 3 then
    listp(T[1]) and integerp(T[1][1]) and is(T[1][1] >= 1) and
    unary_addition_lrt_p(T[2]) and unary_addition_lrt_p(T[3]) and
    is(T[1][1] = (T[2][1][1] + T[3][1][1]))
  else false $

OK: what are the following functions doing?
annotate_unary_add_tree_w_crdv_lrt(T,E) :=
  annotate_unary_add_tree_w_crdv_lrt_r(T,E,1)$
annotate_unary_add_tree_w_crdv_lrt_r(T,E,s) :=
  if not unary_addition_lrt_p(T) then T
OK: what is the purpose of this check??
  else 
    if length(T) = 1 then [E]
    else block([label_l :  T[2][1][1], label_r : T[3][1][1]],
      [create_list(vru_var(s,s+T[1][1]-1,i),i,1,T[1][1]),
       annotate_unary_add_tree_w_crdv_lrt_r(
         T[2],create_list(E[i],i,1,label_l),s),
       annotate_unary_add_tree_w_crdv_lrt_r(
         T[3],
         create_list(E[i],i,label_l+1,label_l+label_r),
         s+label_l)])$
   \endverbatim
